<template>
    <b-form @submit.stop.prevent="submit">
        <b-form-group label-for="theatroSelect">
            Θέατρο
            <b-form-select v-model="theatroSelect" :options="theatraOptions"></b-form-select>
        </b-form-group>
        <b-form-group label-for="sezon">
            Σεζόν
            <b-form-select v-model="sezon" :options="sezonOptions"></b-form-select>
        </b-form-group>
        <b-form-group label-for="enarxi">
            Ημερομηνία Έναρξης
            <b-form-input type="datetime-local" v-model="enarxi"></b-form-input>
        </b-form-group>

        <b-form-group label-for="timi">
            Τιμή Ολόκληρου Εισιτηρίου (€)
            <b-form-input type="number" v-model="timi"></b-form-input>
        </b-form-group>
        ...
        <b-button type="submit">Καταχώρηση</b-button>
    </b-form>
</template>
    ...
    export default {
        ...
        methods: {
            submit: async function(e) {
                ...
                try {
                    const {data:res} = await axios.post('/api/TheatrikiParagwgi/' + this.theatrikiparagwgi + '/Parastasi/store', data);
        ...
        computed: {
            theatraOptions: function() {
                ...
                aithouses.splice(0,0, { value: null, text: 'Επιλέξτε θέατρο και αίθουσα', disabled: true });

                return aithouses;
            },
            sezonOptions: function() {
                let sezon = sezonOptions.map((i) =>
                    ({ text: i, value: i })
                );

                sezon.splice(0,0, { value: null, text: 'Επιλέξτε σεζόν', disabled: true});
